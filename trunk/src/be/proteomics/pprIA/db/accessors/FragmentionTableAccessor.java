...
public class FragmentionTableAccessor implements Deleteable, Retrievable, Updateable, Persistable {
	...
	/**
	 * This variable represents the contents for the 'l_peptideid' column.
	 */
	protected long iL_peptideid = Long.MIN_VALUE;
	...
	/**
	 * This variable represents the key for the 'l_peptideid' column.
	 */
	public static final String L_PEPTIDEID = "L_PEPTIDEID";
	...
	public FragmentionTableAccessor(HashMap aParams) {
		...
		if(aParams.containsKey(L_PEPTIDEID)) {
			this.iL_peptideid = ((Long)aParams.get(L_PEPTIDEID)).longValue();
		}
	...
	/**
	 * This method returns the value for the 'L_peptideid' column
	 * 
	 * @return	long	with the value for the L_peptideid column.
	 */
	public long getL_peptideid() {
		return this.iL_peptideid;
	}
	...
	/**
	 * This method sets the value for the 'L_peptideid' column
	 * 
	 * @param	aL_peptideid	long with the value for the L_peptideid column.
	 */
	public void setL_peptideid(long aL_peptideid) {
		this.iL_peptideid = aL_peptideid;
		this.iUpdated = true;
	}
...
;		}
		...
		while(lRS.next()) {
			...
			iL_peptideid = lRS.getLong("l_peptideid");
	...
	public int update(Connection aConn) throws SQLException {
		...
		PreparedStatement lStat = aConn.prepareStatement("UPDATE fragmention SET fragmentionid = ?, l_peptideid = ?, iontype = ?, ionname = ?, l_ionscoringid = ?, mz = ?, intensity = ?, fragmentionnumber = ?, massdelta = ?, masserrormargin = ?, username = ?, creationdate = ?, modificationdate = CURRENT_TIMESTAMP WHERE fragmentionid = ?");
		lStat.setLong(1, iFragmentionid);
		lStat.setLong(2, iL_peptideid);
	...
	public int persist(Connection aConn) throws SQLException {
		PreparedStatement lStat = aConn.prepareStatement("INSERT INTO fragmention (fragmentionid, l_peptideid, iontype, ionname, l_ionscoringid, mz, intensity, fragmentionnumber, massdelta, masserrormargin, username, creationdate, modificationdate) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_USER, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)");
		if(iFragmentionid == Long.MIN_VALUE) {
			lStat.setNull(1, 4);
		} else {
			lStat.setLong(1, iFragmentionid);
		}
		if(iL_peptideid == Long.MIN_VALUE) {
			lStat.setNull(2, 4);
		} else {
			lStat.setLong(2, iL_peptideid);
		}
