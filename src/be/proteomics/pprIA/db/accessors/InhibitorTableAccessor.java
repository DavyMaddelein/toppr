...
/**
 * This class is a generated accessor for the Inhibitor table.
 *
 * @author DBAccessor generator class (Lennart Martens).
 */
public class InhibitorTableAccessor implements Deleteable, Retrievable, Updateable, Persistable {
	...
	/**
	 * This variable represents the contents for the 'inhibitorid' column.
	 */
	protected long iInhibitorid = Long.MIN_VALUE;
	...
	/**
	 * This variable represents the key for the 'inhibitorid' column.
	 */
	public static final String INHIBITORID = "INHIBITORID";
	 ...
	 * Default constructor.
	 */
	public InhibitorTableAccessor() {
	}

	/**
	 * This constructor allows the creation of the 'InhibitorTableAccessor' object based on a set of values in the HashMap.
	 *
	 * @param	aParams	HashMap with the parameters to initialize this object with.
	 *		<i>Please use only constants defined on this class as keys in the HashMap!</i>
	 */
	public InhibitorTableAccessor(HashMap aParams) {
		if(aParams.containsKey(INHIBITORID)) {
			this.iInhibitorid = ((Long)aParams.get(INHIBITORID)).longValue();
		}
	...
	/**
	 * This method returns the value for the 'Inhibitorid' column
	 * 
	 * @return	long	with the value for the Inhibitorid column.
	 */
	public long getInhibitorid() {
		return this.iInhibitorid;
	}
	...
	/**
	 * This method sets the value for the 'Inhibitorid' column
	 * 
	 * @param	aInhibitorid	long with the value for the Inhibitorid column.
	 */
	public void setInhibitorid(long aInhibitorid) {
		this.iInhibitorid = aInhibitorid;
		this.iUpdated = true;
	}
	...
	public int delete(Connection aConn) throws SQLException {
		PreparedStatement lStat = aConn.prepareStatement("DELETE FROM inhibitor WHERE inhibitorid = ?");
		lStat.setLong(1, iInhibitorid);
	...
	public void retrieve(Connection aConn, HashMap aKeys) throws SQLException {
		...
		if(!aKeys.containsKey(INHIBITORID)) {
			throw new IllegalArgumentException("Primary key field 'INHIBITORID' is missing in HashMap!");
		} else {
			iInhibitorid = ((Long)aKeys.get(INHIBITORID)).longValue()
;		}
		// In getting here, we probably have all we need to continue. So let's...
		PreparedStatement lStat = aConn.prepareStatement("SELECT * FROM inhibitor WHERE inhibitorid = ?");
		lStat.setLong(1, iInhibitorid);
		ResultSet lRS = lStat.executeQuery();
		int hits = 0;
		while(lRS.next()) {
			hits++;
			iInhibitorid = lRS.getLong("inhibitorid");
		...
		lStat.close();
		if(hits>1) {
			throw new SQLException("More than one hit found for the specified primary keys in the 'inhibitor' table! Object is initialized to last row returned.");
		} else if(hits == 0) {
			throw new SQLException("No hits found for the specified primary keys in the 'inhibitor' table! Object is not initialized correctly!");
		}
	}
	...
	public int update(Connection aConn) throws SQLException {
		...
		PreparedStatement lStat = aConn.prepareStatement("UPDATE inhibitor SET inhibitorid = ?, scientific_name = ?, common_name = ?, meropsid = ?, source = ?, username = ?, creationdate = ?, modificationdate = CURRENT_TIMESTAMP WHERE inhibitorid = ?");
		lStat.setLong(1, iInhibitorid);
		lStat.setObject(2, iScientific_name);
		lStat.setObject(3, iCommon_name);
		lStat.setObject(4, iMeropsid);
		lStat.setObject(5, iSource);
		lStat.setObject(6, iUsername);
		lStat.setObject(7, iCreationdate);
		lStat.setLong(8, iInhibitorid);
		int result = lStat.executeUpdate();
		lStat.close();
		this.iUpdated = false;
		return result;
	...
	public int persist(Connection aConn) throws SQLException {
		PreparedStatement lStat = aConn.prepareStatement("INSERT INTO inhibitor (inhibitorid, scientific_name, common_name, meropsid, source, username, creationdate, modificationdate) values(?, ?, ?, ?, ?, CURRENT_USER, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)");
		if(iInhibitorid == Long.MIN_VALUE) {
			lStat.setNull(1, 4);
		} else {
			lStat.setLong(1, iInhibitorid);
		}
