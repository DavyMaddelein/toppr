 ...
 */
public class ProteinSearchServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            // get the protein accession and username
            ...
            searchedParameters = new String[1];
            searchedParameters[0] = protein;
            String uniprotType = req.getParameter("uniprotType");
            if (uniprotType == null) {
                uniprotType = "";
            }
            //if there is no protein given, give an empty search result
            if (protein.equalsIgnoreCase("")) {
                proteinCount = 0;
                siteCount = 0;
            ...
            String query = "";
            if (uniprotType.equalsIgnoreCase("entry_name")) {
                query = "select p.proteinid from protein as p where p.entry_name = '" + protein + "' ";
                ...
                if (uniprotType.equalsIgnoreCase("description")) {
                    query = "select p.proteinid from protein as p where p.name regexp '.*" + protein + ".*' ";
                    query = query + "AND p.proteinid IN (select l_proteinid from peptide_location as l where l.l_groupid in (select d.l_groupid from peptide as d where d.l_projectid IN (select r.projectid from project as r where r.projectid in(select t.l_projectid from project_to_usergroup as t where t.l_usergroupid in (select u.usergroupid from usergroup as u where u.usergroupid in ( select e.l_usergroupid from usergroup_to_groupusers as e where e.l_groupusersid in (select g.groupusersid from groupusers as g where g.groupusersname = '" + userName + "')))))))";
                } else {
                    query = "select p.proteinid from protein as p where p.spaccession = '" + protein + "' ";
                    query = query + "AND p.proteinid IN (select l_proteinid from peptide_location as l where l.l_groupid in (select d.l_groupid from peptide as d where d.l_projectid IN (select r.projectid from project as r where r.projectid in(select t.l_projectid from project_to_usergroup as t where t.l_usergroupid in (select u.usergroupid from usergroup as u where u.usergroupid in ( select e.l_usergroupid from usergroup_to_groupusers as e where e.l_groupusersid in (select g.groupusersid from groupusers as g where g.groupusersname = '" + userName + "')))))))";
                }
            }
            ...
            if (result.length >= 1) {
                ...
                // find  homologues if no protein could be found in the database
                PicrInfoFinder picr = new PicrInfoFinder(protein);
                String[] ncbiAccessions = picr.getNcbiAccessions();
                HomoloGeneIdFinder homologene = new HomoloGeneIdFinder(ncbiAccessions);
                int[] homologeneIds = homologene.getHomologueIds();

                Vector foundHomologues = new Vector();
                for (int i = 0; i < homologeneIds.length; i++) {
                    FoundProtein[] resFound = getHomologueProteins(homologeneIds[i]);
                    for (int j = 0; j < resFound.length; j++) {
                        boolean found = false;
                        for (int k = 0; k < foundHomologues.size(); k++) {
                            FoundProtein proteinAlreadyAdded = (FoundProtein) foundHomologues.get(k);
                            if (resFound[j].getSpAccession().equalsIgnoreCase(proteinAlreadyAdded.getSpAccession())) {
                                found = true;
                            }
                        }
                        if (!found) {
                            foundHomologues.add(resFound[j]);
                        }

                    }
                }
                FoundProtein[] homologues = new FoundProtein[foundHomologues.size()];
                foundHomologues.toArray(homologues);

                if (homologues.length >= 1) {
                    PerformedSearches searches = (PerformedSearches) session.getValue("searches");
                    if (searches == null) {
                        searches = new PerformedSearches();
                    }
                    searches.addSearch(homologues, searchedParameters, true, false, false, proteinCount, siteCount, searchedTreatments, uniquePeptide);
                    proteinCount = 0;
                    siteCount = 0;
                    uniquePeptide = new Vector();
                    session.putValue("searches", searches);
    ...
    private FoundProtein[] getHomologueProteins(int homologeneAccession) {
        ...
        try {
            ...
            for (int i = 0; i < proteinIds.size(); i++) {
                ...
                prep2 = iConn.prepareStatement("Select l.l_groupid, count(l.l_groupid) from peptide_location as l group by l.l_groupid");
                ResultSet rs2 = prep2.executeQuery();
                FoundProtein[] result = this.findProteinResults(rs2, rs1);
                prep1.close();
                rs1.close();
                prep2.close();
                rs2.close();
    ...
    public static void main(String[] args) {
        new ProteinSearchServlet();
    }

   ProteinSearchServlet() {
        String protein = "Q8CGP1";
                // find  homologues if no protein could be found in the database
                PicrInfoFinder picr = new PicrInfoFinder(protein);
                String[] ncbiAccessions = picr.getNcbiAccessions();
                HomoloGeneIdFinder homologene = new HomoloGeneIdFinder(ncbiAccessions);
